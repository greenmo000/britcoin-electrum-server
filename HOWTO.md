...
The most up-to date version of this document is available at:
    ...
    https://github.com/britcoin3/electrum-britcoin-server/blob/master/HOWTO.md
...
document, we assume that user is called 'britcoin'). We also assume the
britcoin user has sudo rights, so we use '$ sudo command' when we need to.
...
you also need to run britcoind and keep a copy of the full blockchain, 
which is roughly 4 GB in April 2014. If you have less than 2 GB of RAM 
make sure you limit britcoind to 8 concurrent connections. If you have more 
...
### Step 1. Create a user for running britcoind and Electrum server

First, create a separate user just for running `britcoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser britcoin
    $ sudo apt-get install git
    $ sudo su - britcoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/britcoin/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download britcoind

We currently recommend britcoind 3.3.0.2.

If you prefer to compile britcoind, here are some pointers for Ubuntu:
(IGNORE THESE, They are out of date.  Remember to use bdb4.8 which you can get from the bitcoin ppa.)

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config automake libtool
    $ sudo su - britcoin
    $ cd ~/src && git clone https://github.com/britcoin-project/britcoin.git -b master-0.10
    $ cd britcoin
    $ ./autogen.sh
    $ ./configure --disable-wallet --without-miniupnpc
    $ make
    $ strip src/britcoind src/britcoin-cli src/britcoin-tx
    $ cp -a src/britcoind src/britcoin-cli src/britcoin-tx ~/bin

### Step 3. Configure and start britcoind

In order to allow Electrum to "talk" to `britcoind`, we need to set up an RPC
username and password for `britcoind`. We will then start `britcoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.britcoin
    $ $EDITOR ~/.britcoin/britcoin.conf

Write this in `britcoin.conf`:
...
If you have an existing installation of britcoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ britcoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `britcoind`:

    $ britcoind

Allow some time to pass, so `britcoind` connects to the network and starts
downloading blocks. You can check its progress by running:

    $ britcoin-cli getinfo

Before starting the electrum server your britcoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start britcoind at boot
time, running as the 'britcoin' user. Check your system documentation to
...
We will download the latest git snapshot for Electrum to configure and install it:
    ...
    $ cd ~
    $ git clone https://github.com/sherlockcoin/electrum-britcoin-server.git
    $ cd electrum-britcoin-server
    $ sudo configure
...
file includes the database setup, britcoind RPC setup, and a few other
options.
...
If you're using user britcoin to run electrum and have added it manually like shown in this HOWTO run 
the following code to add the limits to your /etc/security/limits.conf:

     echo "britcoin hard nofile 65536" >> /etc/security/limits.conf
     echo "britcoin soft nofile 65536" >> /etc/security/limits.conf

Two more things for you to consider:

1. To increase security you may want to close britcoind for incoming connections and connect outbound only

2. Consider restarting britcoind (together with electrum-ltc-server) on a weekly basis to clear out unconfirmed
...
britcoins to confirm that everything is working properly.
